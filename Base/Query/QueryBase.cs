...
namespace ConsoleApplicationSubStat.Base.Query
{
    ...
    public static class QueryBase
    {
        ...
        private static long next_revision_file_id = -1;
        ...
        private static void AddRevisionToDB(DataContext context, RepositoryInfo repoInfo)
        {
            ...
            if(repoInfo.ItemState != RepositoryInfo.RepositoryItemState.Add)
        ...
        static void AddChangedLinesCountOnFile(DataContext context, RepositoryInfo repoInfo, long fileid)
        {
            ...
            DirectoryInfo dirinfo = new DirectoryInfo(ProgramConfiguration.REPOSITORY_FOLDER.FolderPath);
        ...
        private static void AddRevisionFile(DataContext context, RepositoryInfo repoinfo, long fileID)
        {
            ...
            //get maximum ID available and save in variable, so no more DB access will needed    
            if (next_revision_file_id < 0)
            {
                long? ids = revisionfilestable.Max<RevisionFilesDB>(rfd => rfd.ID);
                next_revision_file_id = (ids.HasValue) ? ids.Value + 1 : 0;
            }

            revfileDB.ID = ++next_revision_file_id;
